...
- Train Classifiers on 1/10th of the Tic Tac Toe Data(final, single) (KNN, SVM(Linear, RBF), MLP)
'''
...
# Shuffle the data
data_final, labels_final = shuffle(data_final, labels_final, random_state=0)
data_single, labels_single = shuffle(data_single, labels_single, random_state=0)

# Split the data for 1/10th of the dataset
x_final, x_val_final, y_final, y_val_final = train_test_split(data_final[::10], labels_final[::10], test_size=0.05)
x_single, x_val_single, y_single, y_val_single = train_test_split(data_single[::10], labels_single[::10], test_size=0.05)
